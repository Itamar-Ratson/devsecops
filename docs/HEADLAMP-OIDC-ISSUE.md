...
The Kubernetes API server doesn't validate OIDC tokens by default. Options:
1. Configure API server with OIDC flags (requires cluster recreation, chicken-and-egg with CA)
2. **Use kube-oidc-proxy** (recommended - no cluster changes needed)
...
[GitHub Issue #3143](https://github.com/kubernetes-sigs/headlamp/issues/3143) - affecting versions v0.28.1+

## Solution Implemented: kube-oidc-proxy

Instead of configuring the API server directly, we use [kube-oidc-proxy](https://github.com/jetstack/kube-oidc-proxy):

1. **kube-oidc-proxy** runs as a deployment in the cluster
2. Headlamp is configured to send API requests to kube-oidc-proxy via env vars:
   - `KUBERNETES_SERVICE_HOST=kube-oidc-proxy.kube-oidc-proxy.svc`
   - `KUBERNETES_SERVICE_PORT=443`
3. kube-oidc-proxy validates OIDC tokens and impersonates users to the real API server
4. Keycloak token lifespans extended to mitigate refresh token issues

**Benefits:**
- No API server configuration needed
- Works with any Kubernetes distribution (KinD, EKS, GKE, etc.)
- Terraform-friendly (pure Kubernetes resources)
- Can be managed via GitOps/ArgoCD
